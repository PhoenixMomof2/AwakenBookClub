...
const ReaderForum = () => {
  ...
  const bookConversationCards = books.map(book => (<div key={book.id} className="container-fluid bg-info">
    <div className="row">
      <div className="col bg-warning">
        ...
        <div>{book.comments.map(comment => (
          <div className="col" key={comment.id}>
            <div className="list-group row py-3 bg-dark">
              <div className="list-group-item col" key={comment.id}>
...
// </div> */</div>)
//   // {user.books.map((book) => (<div key={book.id}></div></div>))}
